  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="panel p-5 md:p-6">
        ...
        {draft.moduleTypeId === 'map_node' && (
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            <label><span className="eyebrow">X Coordinate</span><input type="number" className="input mt-2" disabled={draft.locked} value={Number(draft.customFields.coordinateX ?? 0)} onChange={(e) => update('customFields', { ...draft.customFields, coordinateX: Number(e.target.value) })} /></label>
            <label><span className="eyebrow">Y Coordinate</span><input type="number" className="input mt-2" disabled={draft.locked} value={Number(draft.customFields.coordinateY ?? 0)} onChange={(e) => update('customFields', { ...draft.customFields, coordinateY: Number(e.target.value) })} /></label>
            <label><span className="eyebrow">Map Layer</span><input className="input mt-2" disabled={draft.locked} value={String(draft.customFields.mapLayer ?? '')} onChange={(e) => update('customFields', { ...draft.customFields, mapLayer: e.target.value })} placeholder="e.g. Layer 01" /></label>
          </div>
        )}
